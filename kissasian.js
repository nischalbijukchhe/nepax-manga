...
function subtitleLabelFromUrl(url, label) {
    ...
    if (/french|\bfre\b|\bfr[\._-]/i.test(u)) return "French";
    if (/german|\bger\b|\bde[\._-]/i.test(u)) return "German";
    return "Subtitle";
}

function normalizeNetworkSubtitleTracks(rawTracks, headers) {
    const tracks = [];
    const seen = new Set();
    for (const item of rawTracks || []) {
        let url = (item.url || item.file || item.src || item.link || "").trim();
        if (!url) continue;
        if (url.startsWith("//")) url = "https:" + url;
        if (!url.startsWith("http")) continue;
        if (seen.has(url)) continue;
        seen.add(url);
        const label = item.title || item.label || item.lang || item.language || item.name || "";
        tracks.push({
            title: subtitleLabelFromUrl(url, label),
            url,
            headers: headers || {},
        });
    }
...
function parseSubtitleRequests(requests, headers) {
    ...
    for (const raw of requests || []) {
        const url = (raw || "").trim();
        if (!url || !/\.(vtt|srt|ass)(\?|$)/i.test(url)) continue;
        if (seen.has(url)) continue;
        seen.add(url);
        tracks.push({
...
async function resolveEmbedStream(embedUrl, referer) {
    ...
    const networkOptions = {
        ...
        returnHTML: true,
        returnCookies: true,
        waitForSelectors: [".jwplayer", "video", ".plyr", ".video-js"],
        clickSelectors: [".jw-icon-cc", ".jw-icon-settings", ".jw-settings-open"],
    };
    ...
    if (streams.subtitleTracks && streams.subtitleTracks.length) {
        embedSubtitleTracks = mergeSubtitleTrackLists(
            embedSubtitleTracks,
            normalizeNetworkSubtitleTracks(streams.subtitleTracks, headers)
        );
    }
    ...
    if (streams.html) {
        embedSubtitleTracks = mergeSubtitleTrackLists(
            embedSubtitleTracks,
            parseSubtitleTracks(streams.html, headers)
        );
    }
...
async function extractStreamUrl(url) {
    try {
        ...
        const englishSub =
            subtitleTracks.find((t) => {
                const title = (t.title || "").trim();
                return /english|^en$|^eng$/i.test(title);
            }) ||
            subtitleTracks[0];
