...
function parseBsArticles(html, limit) {
    ...
    while ((articleMatch = articleRegex.exec(html)) !== null) {
        ...
        const title = decodeHtmlEntities(
            ...
            .trim();
        const imgMatch =
            block.match(/<img[^>]+src="([^"]+)"[^>]*itemprop="image"/i) ||
...
async function browseSections() {
    try {
        ...
        const genreFetches = [
            ...
            { id: "drama", title: "Drama Genre", path: "/genres/drama/" },
            { id: "thriller", title: "Thriller", path: "/genres/thriller/" },
        ];

        const genreResults = await Promise.all(genreFetches.map((g) => fetchGenreSection(g)));
        for (const section of genreResults) {
...
async function extractStreamUrl(url) {
    try {
        ...
        for (const pageUrl of pagesToTry) {
            embedJobs.push(
                (async () => {
                    try {
                        const html = await fetchHtml(pageUrl);
                        return parseIframeSources(html).map((embedUrl) => ({
                            embedUrl,
                            pageUrl,
                        }));
                    } catch (e) {
        ...
        const embedLists = await Promise.all(embedJobs);
        const flatEmbeds = embedLists.flat();

        const resolveJobs = flatEmbeds.map(({ embedUrl }) =>
            resolveEmbedStream(embedUrl, episodeUrl).then((resolved) => ({
                embedUrl,
                resolved,
            }))
        );

        const resolvedList = await Promise.all(resolveJobs);
        const streams = [];
        let serverIndex = 0;
