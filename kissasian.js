...
function parseEpisodesFromSeriesHtml(html) {
    ...
    while ((match = linkRegex.exec(block)) !== null) {
        ...
        if (number == null && !href.match(/kissasian\.cam\/[^/]+\/?$/i)) continue;
        seen.add(href);
...
async function resolveFirstEmbed(embedUrls, referer) {
    ...
    for (const embedUrl of urls) {
        try {
            let resolved = await resolveEmbedStream(embedUrl, referer, { fast: false });
            if (!resolved || !resolved.streamUrl) {
                resolved = await resolveEmbedStream(embedUrl, referer, { fast: true });
            }
            if (resolved && resolved.streamUrl) {
...
async function extractStreamUrl(url) {
    try {
        ...
        for (const pageUrl of pagesToTry.slice(0, 4)) {
        ...
        if (!streams.length) {
            for (const pageUrl of pagesToTry.slice(0, 4)) {
                ...
                await tryResolveFromPage(pageUrl, 3);
            }
        }
