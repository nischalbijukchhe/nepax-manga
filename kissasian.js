...
async function extractStreamUrl(url) {
    try {
        ...
        const pagesToTry = (mirrorPages.length ? orderMirrorPages(mirrorPages) : [episodeUrl]).slice(0, 2);

        function embedPriority(embedUrl) {
            const u = embedUrl.toLowerCase();
            if (u.includes("vidmoly") || u.includes("mixdrop") || u.includes("mxdrop")) return 0;
            if (u.includes("dood") || u.includes("highload")) return 1;
            if (u.includes("justplay") || u.includes("strcloud")) return 2;
            return 3;
        }
        ...
        for (const pageUrl of pagesToTry) {
            ...
            const iframes = parseIframeSources(html)
                .sort((a, b) => embedPriority(a) - embedPriority(b))
                .slice(0, 2);
