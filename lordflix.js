...
function buildSnowhouseUrl({ title, type, year, imdbId, tmdbId, season, episode, server }) {
    // JavaScriptCore has no URLSearchParams — build query manually.
    const parts = [
        `title=${encodeQueryValue(title)}`,
        `type=${encodeQueryValue(type)}`,
        `server=${encodeQueryValue(server)}`,
    ];
    if (year) parts.push(`year=${encodeQueryValue(year)}`);
    if (imdbId) parts.push(`imdb=${encodeQueryValue(imdbId)}`);
    if (tmdbId) parts.push(`tmdb=${encodeQueryValue(tmdbId)}`);
    if (season) parts.push(`season=${encodeQueryValue(season)}`);
    if (episode) parts.push(`episode=${encodeQueryValue(episode)}`);
    return `https://snowhouse.lordflix.club/?${parts.join("&")}`;
}
