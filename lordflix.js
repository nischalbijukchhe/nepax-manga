...
// v1.1.3 — valid enc-dec servers, videasy metadata, API error surfacing

const TMDB_KEY = "9801b6b0548ad57581d111ea690c85c8";
const TMDB_PROXY = "https://post-eosin.vercel.app/api/proxy?url=";
// enc-dec.app lordflix accepts these server names (London/Paris/Raleigh are invalid).
const LORDFLIX_SERVERS = [
    "Berlin", "Warsaw", "Tokyo", "New York", "Amsterdam",
    "Sydney", "Mumbai", "Singapore", "Dallas", "Chicago",
];
...
// Match upstream lordflix.js: title is encodeURIComponent'd once in the snowhouse URL.
function buildSnowhouseUrl({ title, type, year, imdbId, tmdbId, season, episode, server }) {
    const encTitle = encodeURIComponent(String(title || ""));
    const y = year || "";
    const imdb = imdbId || "";
    const tmdb = tmdbId || "";
    let url =
    ...
    for (const streamType of typesToTry) {
        for (const server of LORDFLIX_SERVERS) {
            try {
                ...
                console.log(`LordFlix ${server} (${streamType}) failed: ${msg}`);
                // Stop trying more servers if API says content is unavailable.
                if (/not available|no playable|not found|premier/i.test(msg)) {
                    break;
                }
            }
        }
