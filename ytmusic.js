...
function pickBestAudioFormat(formats) {
    ...
    audio.sort((a, b) => {
        const pref = (x) => {
            ...
            if (itag === 251) return 1000;
            if (itag === 140) return 900;
...
function streamsFromPlayerData(data, clientName) {
    ...
    // Prefer direct audio URLs (m4a/webm) — AVPlayer plays these reliably with Referer headers.
    const formats = [
...
async function fetchFromWatchPage(videoId, host) {
    ...
    try {
        ...
        const result = streamsFromPlayerData(player, label);
...
async function fetchPlayerStream(videoId) {
    ...
    // 1) Watch page embed data (most reliable without login)
    for (const host of [YT_HOST, YTM_HOST]) {
    ...
    // 2) InnerTube player API (few clients)
    const payload = { videoId, contentCheckOk: true, racyCheckOk: true };
    const attempts = buildPlayerAttempts(videoId).filter((a) =>
        a.name === "YT_TV"
        || a.name === "YT_ANDROID"
        || a.name === "YT_IOS"
        || a.name === "YTM_ANDROID_MUSIC"
        || a.name === "YTM_ANDROID"
        || a.name === "YTM_WEB"
    );
