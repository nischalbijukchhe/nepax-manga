...
/** InnerTube filter: artists only */
const YTM_ARTIST_SEARCH_PARAMS = "EgWKAQIIAWoKEAEQBRAE4AQA%3D%3D";
...
function hrefForNavigation(nav) {
    ...
    if (nav.browseEndpoint) {
        ...
        if (bid) {
            ...
            if (lower.startsWith("uc") || lower.startsWith("mpreb_") || lower.startsWith("mpla")) {
                return `${YTM_HOST}/channel/${bid}`;
            }
            return `${YTM_HOST}/browse/${bid}`;
        }
    }
    return null;
}

function isArtistHref(href) {
    const h = String(href || "").toLowerCase();
    if (!h) return false;
    if (h.includes("/channel/")) return true;
    if (h.includes("mpreb_") || h.includes("mpla")) return true;
    if (h.includes("/browse/") && (h.includes("/uc") || h.includes("mpreb"))) return true;
    return false;
}

function isArtistLike(type, href, sectionTitle) {
    if (type === "artist") return true;
    const section = String(sectionTitle || "").toLowerCase();
    if (section.includes("artist")) return true;
    return isArtistHref(href);
}
...
function nameMatchesQuery(name, query) {
    ...
    return n.split(/\s+/).some((w) => w.startsWith(q) || w === q);
}
...
function inferMusicType(href, sectionTitle) {
    ...
    if (isArtistHref(h)) return "artist";
    if (h.includes("/browse/")) return "album";
    return "song";
}
...
async function searchResults(keyword) {
    try {
        ...
        function pushArtist(raw) {
            ...
            if (!nameMatchesQuery(title, query)) return;
            const type = isArtistLike(raw.type, href, raw.section) ? "artist" : inferMusicType(href, raw.section);
            if (type !== "artist" && !isArtistHref(href)) return;
            seenHref.add(href);
            final.push({
                title,
                subtitle: raw.subtitle || "Artist",
                image: raw.image || "",
                href,
                type: "artist",
                videoId: "",
                section: raw.section || "Artists"
            });
        }

        // 1) Artists (so typing any letter shows matching artist names)
        try {
            const artistData = await ytmPost("search", { query, params: YTM_ARTIST_SEARCH_PARAMS });
            const artistRaw = [];
            walkCollectSearch(artistData, artistRaw, new Set());
            for (const raw of artistRaw) {
                pushArtist(raw);
            }
        } catch (e) {
            console.log("YTM artist search: " + e);
        }

        // 2) Songs from YouTube Music (best for play + download)
        try {
            const songData = await ytmPost("search", {
                query,
                params: YTM_SONG_SEARCH_PARAMS
        ...
        // 3) Videos section (some tracks appear here)
        try {
        ...
        // 4) General YTM search — albums, artists, playlists (not for direct stream)
        try {
            ...
            for (const raw of allRaw) {
                ...
                if (id) {
                    pushSong(raw);
                } else if (isArtistLike(raw.type, raw.href, raw.section)) {
                    pushArtist(raw);
                } else {
                    pushOther(raw);
                }
        ...
        const artists = final.filter((x) => (x.type || "") === "artist");
        const songs = final.filter((x) => x.videoId && String(x.videoId).length === 11);
        const rest = final.filter((x) => (x.type || "") !== "artist" && !(x.videoId && String(x.videoId).length === 11));
        return JSON.stringify([...artists, ...songs, ...rest].slice(0, 60));
    } catch (error) {
...
async function musicBrowseSongs(url) {
    try {
        const out = [];
        const seen = new Set();
        const playlistId = extractPlaylistId(url);
        const browseId = extractBrowseId(url) || (playlistId ? playlistToBrowseId(playlistId) : null);
        if (!browseId) return JSON.stringify([]);
        ...
        return JSON.stringify(songs.slice(0, 80));
    } catch (error) {
        console.log("YTM musicBrowseSongs error: " + error);
        return JSON.stringify([]);
    }
}
...
function pickBestAudioFormat(formats) {
    ...
    audio.sort((a, b) => {
        const pref = (x) => {
            ...
            let score = br;
            if (itag === 140 || mime.includes("mp4") || mime.includes("mpeg")) score += 500000;
...
function streamsDirectAudioOnly(data, clientName) {
    ...
    const picked = pickBestPlayableFormat(formats);
    if (picked && picked.streamUrl) {
        ...
        return streamResultFromUrl(`${q} · ${mime || "stream"}`, picked.streamUrl);
    }
