...
/** Full-size artwork for player UI (maxres / large googleusercontent). */
function hdThumbnailUrl(url, videoId) {
    ...
    if (url.indexOf("googleusercontent.com") !== -1 || url.indexOf("ggpht.com") !== -1) {
        return url
            .replace(/=w\d+-h\d+(-[a-z0-9-]+)?/gi, "=w1200-h1200")
            .replace(/=s\d+(-[a-z0-9-]+)?/gi, "=s1200");
    }
    if (url.indexOf("ytimg.com") !== -1) {
        return url.replace(/\/[a-z]+default\.jpg/i, "/maxresdefault.jpg");
    }
    return url;
}
...
function parseResponsiveListItem(renderer, out, seen, sectionTitle) {
    ...
    out.push({
        title: title || "Unknown",
        subtitle: subtitle || "",
        image: hdThumbnailUrl(bestThumbnail(thumbs), videoId),
...
function normalizeSongEntry(raw) {
    ...
    return {
        ...
        image: hdThumbnailUrl(raw.image || "", videoId),
...
async function collectSongsForQuery(keyword) {
...
async function searchResultsSongsOnly(keyword) {
    try {
        return JSON.stringify(await collectSongsForQuery(keyword));
    } catch (error) {
        console.log("searchResultsSongsOnly error: " + error);
        return JSON.stringify([]);
    }
}

async function searchResultsBatch(keywords) {
    try {
        const list = Array.isArray(keywords) ? keywords : JSON.parse(String(keywords || "[]"));
        const out = {};
        const concurrency = 4;
        for (let i = 0; i < list.length; i += concurrency) {
            ...
            const settled = await Promise.all(
                slice.map(async (q) => {
                    ...
                    try {
                        return { query, items: await collectSongsForQuery(query) };
                    } catch (e) {
                        return { query, items: [] };
                    }
                })
            ...
            for (const row of settled) {
                out[row.query] = row.items;
            }
        }
        return JSON.stringify(out);
    } catch (error) {
        console.log("searchResultsBatch error: " + error);
        return JSON.stringify({});
    }
}
    ...
    try {
        ...
        function pushOther(raw) {
            ...
            final.push({
                title: raw.title || "Unknown",
                subtitle: raw.subtitle || "",
                image: hdThumbnailUrl(raw.image || "", id),
                href,
        ...
        try {
            const songOnly = await collectSongsForQuery(query);
            for (const song of songOnly) {
...
async function spotifyEmbedPlaylistTrackList(playlistId) {
    try {
        ...
        return spotifyTracksFromEmbedHTML(html, playlistId);
    } catch (e) {
...
async function spotifyPlaylistTrackList(playlistUrl) {
    try {
        ...
        if (!playlistId) {
            return JSON.stringify({
                error: "Paste a valid Spotify playlist link (open.spotify.com/playlist/…)."
            });
        }

        const embedData = await spotifyEmbedPlaylistTrackList(playlistId);
