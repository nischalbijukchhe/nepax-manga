...
async function searchResults(keyword) {
    try {
        ...
        // 4) Artist filter (surfaces channel / artist rows like Spotify search)
        try {
            const artistParams = "EgIQAgIoAQ%3D%3D";
            const artistData = await ytmPost("search", { query, params: artistParams });
            const artistRaw = [];
            walkCollectSearch(artistData, artistRaw, new Set());
            for (const raw of artistRaw) {
                if ((raw.type || inferMusicType(raw.href, raw.section)) === "artist") {
                    pushOther({ ...raw, type: "artist", section: raw.section || "Artists" });
                }
            }
        } catch (e) {
        ...
        return JSON.stringify([...artists, ...songs, ...rest].slice(0, 50));
    } catch (error) {
...
/** Songs on an artist / album / playlist browse page (titles + artwork). */
