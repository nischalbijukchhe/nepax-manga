...
function audioStreamTitle(format) {
    const itag = parseInt(format.itag, 10);
    const br =
        parseInt(format.bitrate, 10) ||
        parseInt(format.averageBitrate, 10) ||
        0;
    const kbps = br > 0 ? Math.round(br / 1000) : 0;
    if (itag === 141) return `High · ${kbps || 256} kbps AAC`;
    if (itag === 140) return `High · ${kbps || 128} kbps AAC`;
    if (itag === 139) return `Audio · ${kbps || 48} kbps AAC`;
    const q = format.audioQuality || format.quality || "Audio";
    const mime = (format.mimeType || "").split(";")[0];
    return kbps > 0 ? `${q} · ${kbps} kbps · ${mime || "m4a"}` : `${q} · ${mime || "stream"}`;
}
...
function streamsDirectAudioOnly(data, clientName) {
    ...
    for (const entry of picked) {
        ...
        streams.push({
            title: audioStreamTitle(entry.format),
