...
function pickBestAudioFormat(formats) {
    ...
    audio.sort((a, b) => {
        const pref = (x) => {
            ...
            // iOS AVPlayer plays m4a/aac (140) — not webm/opus (251).
            if (itag === 140 || mime.includes("mp4") || mime.includes("mpeg")) return 3000;
            if (itag === 139) return 2500;
            if (itag === 251 || mime.includes("webm") || mime.includes("opus")) return 200;
            return 1000 + (parseInt(x.format.bitrate, 10) || 0);
        };
