// Animepahe — NEPAX/Shirox bundled (v1.6.0)
// animepahe.pw — WebView Cloudflare bypass, native API after cf_clearance
...
function hasCfClearance() {
    ...
    return ck.includes("cf_clearance=");
}

function hasCfCookies() {
    const ck = paheCookieHeader().toLowerCase();
    return hasCfClearance() || ck.includes("__cf_bm=");
}
...
async function warmPaheSession(force) {
    ...
    _paheSession.warming = (async function () {
        try {
            ...
            if (hasCfClearance() && (!html || !isBlockedResponse(html))) {
                markSessionWarm();
            } else {
...
async function paheFetch(url, options) {
    options = options || {};
    if (!isSessionFresh() && !hasCfClearance()) {
    ...
    let text = null;
    if (hasCfClearance() && !options.forceWebView) {
    ...
    if (!isBlockedResponse(text) && hasCfClearance()) {
        markSessionWarm();
    }
    return {
        text: async function () {
...
async function paheFetchJson(url, options) {
    ...
    if (hasCfClearance()) {
        const native = await paheTryNativeFetch(abs, headers);
        if (native) {
            try {
    ...
    const res = await paheFetch(abs, options);
    const text = await res.text();
...
async function warmAnimepaheSession(force) {
    ...
    return _paheSession.warmed || hasCfClearance() ? "ok" : "blocked";
}
...
(function (g) {
    ...
    g.mergePaheCookies = mergePaheCookies;
