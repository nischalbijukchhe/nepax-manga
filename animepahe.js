// Animepahe — NEPAX/Shirox bundled (v1.5.2)
// animepahe.pw — WebView-first API (Cloudflare), multi-query search fallbacks
...
async function warmPaheSession(force) {
    if (!force && _paheSession.warmed) return;
...
async function paheFetchViaWebView(url, headers, opts) {
    ...
    const result = await networkFetch(url, {
        ...
        returnHTML: true,
        returnCookies: true,
        maxWaitTime: opts.maxWaitTime || 28,
...
async function paheFetch(url, options) {
    ...
    const isApi = abs.indexOf("/api?") !== -1;

    if (isApi) {
        let html = await paheFetchViaWebView(abs, headers, options);
        if (isBlockedResponse(html)) {
            _paheSession.warmed = false;
            await warmPaheSession(true);
            html = await paheFetchViaWebView(abs, headers, options);
        }
        return {
            text: async function () {
                return html;
    ...
    let text = await paheFetchViaWebView(abs, headers, options);
...
async function warmAnimepaheSession() {
    await warmPaheSession(true);
    return "ok";
}

async function searchResults(keyword) {
    const queries = buildAnimepaheSearchQueries(keyword);
    let lastError = "";
    ...
    for (let pass = 0; pass < 2; pass++) {
        for (let qi = 0; qi < queries.length; qi++) {
            ...
            try {
                await warmPaheSession(pass > 0 || qi > 0);
                ...
                console.log("Animepahe search try '" + query + "' pass " + pass + ": " + lastError);
                _paheSession.warmed = false;
            }
        }
    }
